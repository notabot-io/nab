...
DNAB is a container-centric, lightweight, automation server written in Go. DNAB aims to take the best parts of other tools to create a powerful, flexible, and easy-to-use tool.

## Motivation

The inspiration for DNAB comes from a few places in particular; Concourse, Drone, Jenkins, and Travis. All of these tools are excellent, but none of them perfectly achieved what DNAB sets out to achieve as an automation tool. A lot of the things that these tools do very well has been looked at and learnt from, and DNAB sets out to solve as many of the problems with them as it can. This mainly boils down to:

* **Configuration**: DNAB aims to  have a simple but flexible declarative configuration syntax that you keep as code in a repository that is applied to a server (think Kubernetes).
* **Resource Usage**: DNAB aims to be easy and cheap to host. Resource usage needs to be kept to a minimum. A $5p/m VPS should be all you need to get started. It should grow as you need it to.
* **Portability**: DNAB also aims to be simple to set up, and easy to deploy in a wide range of environments. It may not be perfect for everything, but it hopes to cover a lot of bases.
...
**Configuration** in Concourse is very declarative. You don't have to worry about your CI server dying because all of it's configuration is just configuration files. Great! The prlblems come in when you want to express some things more concisely. There are no variables - by design, but this can be a hinderance and instead leads to much more verbose pipelines, or lots of custom resources being created.
...
**Configuration** is hoarded by Jenkins. Jenkins has your configuration, and it's a bit trickier to keep configuration as code - even with the new pipelines plugin. You still have to configure a lot in the jobs themselves - these can be kept as just the XML config files, but then what about all of the other things that come in to interact with the jobs.
